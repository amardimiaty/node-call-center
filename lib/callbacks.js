...
async function callToNextAgent(callId, ctx) {
	...
	try {
		agentCallId = (await ctx.bandwidthApi.Call.create({from: data.phoneNumber, to: phoneNumber, tag: callId, callbackUrl: `https://${process.env.HOST}/bandwidth/callback/call`})).id;
	} catch (err) {
